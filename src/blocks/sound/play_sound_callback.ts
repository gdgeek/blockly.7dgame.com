...
const block: BlockDefinition = {
  ...
  getBlockJson(_parameters: unknown): object {
    const json = {
      type: "block_type",
      message0: (Blockly.Msg as unknown as Record<string, Record<string, string>>)["SOUND_PLAY"][window.lg],
      args0: [
        {
          type: "input_value",
          name: "sound",
  ...
  getJavascript(_parameters: unknown): (block: BlocklyBlock, generator: BlocklyGenerator) => string {
    const script = function (_block: BlocklyBlock, generator: BlocklyGenerator): string {
  ...
  getLua(_parameters: unknown): (block: BlocklyBlock, generator: BlocklyGenerator) => string {
    const lua = function (_block: BlocklyBlock, generator: BlocklyGenerator): string {
      const value_sound = generator.valueToCode(
        _block,
        "sound",
        generator.ORDER_NONE

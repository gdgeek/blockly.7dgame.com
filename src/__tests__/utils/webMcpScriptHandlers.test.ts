...
const createHandlers = (
  ...
  const handlers = createWebMcpScriptRequestHandlers({
    ...
    onMutationSettled: onWorkspaceChange,
  });
  return { handlers, onWorkspaceChange };
};

describe("Blockly WebMCP script handlers", () => {
  it("bounds UTF-8 input bytes and leaves the live workspace unchanged after a failed batch", () => {
    const workspace = new Blockly.Workspace();
    workspace.newBlock("webmcp_test_event");
    const before = saveWorkspace(workspace);
    const { handlers } = createHandlers(workspace);
    expect(
      handlers["webmcp-stage-meta-script-replace"]({
        workspace: { data: "中".repeat(180_000) },
      })
    ).toMatchObject({ ok: false, code: "INVALID_RANGE" });
    expect(
      handlers["webmcp-stage-script-block-batch"]({
        operations: [
          {
            op: "create",
            block: { type: "webmcp_test_event", data: "中".repeat(90_000) },
          },
        ],
      })
    ).toMatchObject({ ok: false, code: "INVALID_RANGE" });
    expect(
      handlers["webmcp-stage-script-block-batch"]({
        operations: [
          { op: "create", block: { type: "webmcp_test_next" } },
          { op: "delete", blockId: "missing" },
        ],
      })
    ).toMatchObject({ ok: false });
    expect(saveWorkspace(workspace)).toEqual(before);
    workspace.dispose();
  });
  it("returns warnings without blocking recoverable workspaces and reports both generation failures", () => {
    const workspace = new Blockly.Workspace();
    workspace.newBlock("webmcp_test_event");
    const { generateAll, generateAllSafely } = useCodeGenerator();
    const handlers = createWebMcpScriptRequestHandlers({
      getWorkspace: () => workspace,
      saveWorkspace,
      loadWorkspace: (state, target) =>
    ...
    const js = vi
      .spyOn(javascriptGenerator, "workspaceToCode")
      .mockImplementation(() => {
        throw new Error("JS failed");
      });
    const lua = vi
      .spyOn(luaGenerator, "workspaceToCode")
      .mockImplementation(() => {
        throw new Error("Lua failed");
      });
    try {
      expect(handlers["webmcp-validate-meta-script"]({})).toMatchObject({
        ok: true,
        valid: false,
        canSave: false,
        warnings: [
  ...
  it("serializes write completion and drops its result when the active generation changes", async () => {
    const workspace = new Blockly.Workspace();
    workspace.newBlock("webmcp_test_event");
    let generation = 1;
    const settled = vi.fn();
    ...
    const stage = handlers["webmcp-stage-meta-script-replace"]({
      workspace: workspaceData("webmcp_test_next"),
    }) as {
    ...
    const payload = {
      workspaceVersion: stage.workspaceVersion,
      workspace: stage.proposedWorkspace,
    };
    const completing = handlers["webmcp-complete-meta-script-replace"](payload);
    expect(
      await handlers["webmcp-complete-meta-script-replace"](payload)
    ).toMatchObject({ ok: false, code: "SCRIPT_BUSY" });
    generation += 1;
  ...
  it("rejects invalid candidates and detects version conflicts", async () => {
    ...
    expect(
      handlers["webmcp-stage-meta-script-replace"]({
        workspace: { blocks: { blocks: [{ type: "webmcp_unknown_block" }] } },
      })
    ).toMatchObject({ ok: false });

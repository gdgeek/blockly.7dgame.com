...
const clearWorkspaceVariables = (workspace: Blockly.Workspace) => {
...
const replaceWorkspaceWithUndo = (
  workspace: Blockly.WorkspaceSvg,
  candidate: JsonRecord,
  options: WebMcpScriptHandlerOptions
) => {
  const previous = options.saveWorkspace(workspace);
  const groupId = `webmcp-script-${Date.now()}-${Math.random()
    .toString(36)
    .slice(2, 8)}`;
  workspace.removeChangeListener(options.onWorkspaceChange);
  let groupStarted = false;
  try {
    Blockly.Events.setGroup(groupId);
    groupStarted = true;
    workspace.clear();
    clearWorkspaceVariables(workspace);
    Blockly.serialization.workspaces.load(candidate, workspace, {
      recordUndo: true,
    });
  } catch (error) {
    if (groupStarted) Blockly.Events.setGroup(false);
    groupStarted = false;
    try {
    ...
    if (groupStarted) Blockly.Events.setGroup(false);
    workspace.addChangeListener(options.onWorkspaceChange);
...
const applyScriptBlockBatch = (
  ...
  return operations.map((operation, index) => {
    ...
    if (op === "set_fields") {
      ...
      return { index, op, blockId: block.id, fields: applied };
    }
    if (op === "set_state") {
      let changed = false;
    ...
    if (op === "move") {
      ...
      return { index, op, blockId: block.id, position };
    }
    if (op === "connect") {
...
export const createWebMcpScriptRequestHandlers = (
  ...
  const validateScript = (payload: JsonRecord) => {
    try {
      const workspace = requireWorkspace(options);
      const inspected = inspectWorkspace(workspace, options);
      if (payload.focusIssue && inspected.rawIssue) {
  ...
  const completeScriptReplace = async (payload: JsonRecord) => {
    try {
      ...
      replaceWorkspaceWithUndo(workspace, proposed.workspaceData, options);
